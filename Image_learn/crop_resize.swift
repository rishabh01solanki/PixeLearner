...
//  Image_learn
//
//  Created by Rishabh Solanki on 9/11/23.
//
...
func squareBoundingBox(from observation: VNFaceObservation, in ciImage: CIImage) -> CGRect {
    ...
    let faceRect = CGRect(
        ...
        y: (1 - observation.boundingBox.maxY) * ciImage.extent.size.height,
    ...
    let sideLength = min(faceRect.width, faceRect.height)
...
func pixelBufferToUIImage(pixelBuffer: CVPixelBuffer) -> UIImage? {
    ...
    guard let cgImage = context.createCGImage(ciImage, from: CGRect(origin: .zero, size: ciImage.extent.size)) else {
        return nil
    }
